...
int newOpNode(int i, Node *parent){
    ...
    opNode->opRoot = 1; //helps with printing
...
void printCallStmt(FILE *opf, Node *callStmt){
    ...
    fprintf(opf, "          [%s]\n", callStmt->children[0]->ID); // [foo

    Node *args = callStmt->children[1];


    fprintf(opf, "          [%s", args->ID); // [args

    int j = 0;
    while(args->children[j] != NULL){
        printPostOrder(opf, args->children[j]);
        j++;
    }
...
Node * parse(int argc, char *argv[]){
    ...
    if((ifp = fopen(argv[1], "r")) == NULL) {
        errorExit("parser - error opening input file", -1);
    }

    if((ofp = fopen("parser_output", "w+")) == NULL){
        errorExit("parser - error opening output file", -1);
    }
